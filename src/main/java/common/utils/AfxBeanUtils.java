...
import common.annotation.BeanUtilsNoCopyValue;
...
public class AfxBeanUtils {
    ...
    public static void copySamePropertyValue(Object src, Object des, boolean isNotCopyNull, boolean isOverrideDesNotNullValue) {
        Field[] srcFiledArr = AfxBeanUtils.getAllFields(src.getClass());
        for (int i = 0; i < srcFiledArr.length; i++) {
            ...
            if (null != desField) {
                try {// 类型不同，则不进行复制
                    ...
                    if (isNotCopyNull) {// 是否为null不复制,默认为false
                        /**
                         * Edit by bin. 20140321 null值不复制
                         */
                        if (srcValue == null) {
                            continue;
                        }
    ...
    public static Object getFiledValue(Object target, String fieldName) {
        Object obj = null;
        try {
            Field field = AfxBeanUtils.getField(target.getClass(), fieldName);
    ...
    public static void copySamePropertyValue(Object src, Object des) {
        AfxBeanUtils.copySamePropertyValue(src, des, false);
    }
